...
@endsection
@section('content')
    <form id="kt_form" class="form row d-flex flex-column flex-lg-row addForm" data-kt-redirect="{{ route('admin.adds.index') }}"
            action="{{ isset($add) ? route('admin.adds.update' ,  $add->id) : route('admin.adds.store') }}">
        @csrf
        @isset($add)
            @method('PATCH')
        @endif
                    ...
                    <div class="card-toolbar">
                        <label class="form-check form-switch form-check-custom form-check-solid">
                            ...
                            <input class="form-check-input btn active_operation" style="margin: 0 auto;" type="checkbox"
                                name="status" @if(isset($add) && $add->status == 'active') checked="checked" @endif value="active" >
                ...
                <div class="card-header card-header justify-content-center p-5">
                    <div class="card-toolbar">
                        <div class="image-input image-input-outline image-input-placeholder image-input-empty" data-kt-image-input="true">
                            ...
                            <div class="image-input-wrapper w-200px h-200px" style="background-image: url(@if(isset($add) && $add->image !== null) {{ asset($add->image) }} @else {{ asset('admin_assets/media/svg/avatars/blank.svg') }} @endif)"></div>
                                    ...
                                    <div class="col-md-12 fv-row fv-plugins-icon-container ">
                                        <div class="mb-5 fv-row">
                                            ...
                                            <input type="text" name="title_ar" class="form-control mb-2 w-100"
                                                ...
                                                value="{{ isset($add) ? $add->translate('ar')->title : '' }}" />
                                        </div>
                                    </div>
                                </div>
                                <div>
                                    <label class="form-label">{{ __('admin.global.category_description') }} </label>
                                    <textarea name="description_ar" id="summernote">{{ isset($add) ? $add->translate('ar')->description : '' }}</textarea>
                                </div>
                                <div>
                                    <label class="form-label">{{ __('admin.global.category_description') }} </label>
                                    <textarea name="short_description_ar" id="summernote">{{ isset($add) ? $add->translate('ar')->short_description : '' }}</textarea>
                                </div>
                            </div>
                                    ...
                                    <div class="col-md-12 fv-row fv-plugins-icon-container ">
                                        <div class="mb-5 fv-row">
                                            ...
                                            <input type="text" name="title_en" id="title_en" class="form-control mb-2"
                                                placeholder="{{ __('admin.global.category_name') }}"
                                                value="{{ isset($add) ? $add->translate('en')->title : '' }}" />
                                        </div>
                                    </div>
                                </div>
                                <div>
                                    <label class="form-label">{{ __('admin.global.category_description') }}</label>
                                    <textarea name="description_en" id="summernote_en">{{ isset($add) ? $add->translate('en')->description : '' }}</textarea>
                                </div>
                                <div>
                                    <label class="form-label">{{ __('admin.global.category_description') }}</label>
                                    <textarea name="short_description_en" id="summernote_en">{{ isset($add) ? $add->translate('en')->short_description : '' }}</textarea>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        <div class="page-buttuns mt-5">
            <div class="row justify-content-between">
                <div class="d-flex justify-content-end right">
                    ...
                    <a href="{{ route('admin.adds.index') }}" id="kt_ecommerce_add_product_cancel"
