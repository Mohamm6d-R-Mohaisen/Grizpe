@extends('admin.layouts.master', ['is_active_parent' => 'reviews','is_active'=> 'reviews'])
@section('title')
    {{ __('admin.global.add_new_category') }}
@endsection
@section('content')
    <form id="kt_form" class="form row d-flex flex-column flex-lg-row addForm" data-kt-redirect="{{ route('admin.reviews.index') }}"
                    ...
                    <div class="card-toolbar">
                        <label class="form-check form-switch form-check-custom form-check-solid">
                            ...
                            <input class="form-check-input btn active_operation" style="margin: 0 auto;" type="checkbox"
                                   name="status" @if(isset($review) && $review->status == 'active') checked="checked" @endif value="active" >
                ...
                <div class="card-header card-header justify-content-center p-5">
                    <div class="card-toolbar">
                        <div class="image-input image-input-outline image-input-placeholder image-input-empty" data-kt-image-input="true">
                            ...
                            <div class="image-input-wrapper w-200px h-200px" style="background-image: url(@if(isset($review) && $review->image !== null) {{ asset($review->image) }} @else {{ asset('admin_assets/media/svg/avatars/blank.svg') }} @endif)"></div>
        ...
        <div class="d-flex flex-column flex-row-fluid gap-3 col-lg-9">
            <div class="d-flex flex-column gap-5">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-12 fv-row fv-plugins-icon-container ">
                                        <div class="mb-5 fv-row">
                                            <label class="required form-label">{{ __('admin.global.name') }}</label>
                                            <input type="text" name="name_ar" class="form-control mb-2 w-100"
                                                   placeholder="{{ __('admin.global.enter_name') }}"
                                                   value="{{ isset($review) ? $review->translate('ar')->name : '' }}" />
                                        </div>
                                    </div>
                                </div>
                                <div>
                                    <label class="form-label">{{ __('admin.global.category_description') }} </label>
                                    <textarea name="comment_ar"  class="form-control">{{ isset($review) ? $review->translate('ar')->comment : '' }}</textarea>
                                </div>
                            </div>
                                    ...
                                    <div class="col-md-12 fv-row fv-plugins-icon-container ">
                                        <div class="mb-5 fv-row">
                                            ...
                                            <input type="text" name="name_en" id="name_en" class="form-control mb-2"
                                                   placeholder="{{ __('admin.global.enter_name') }}"
                                                   value="{{ isset($review) ? $review->translate('en')->name : '' }}" />
                                        </div>
                                    </div>
                                </div>
                                <div>
                                    <label class="form-label">{{ __('admin.global.category_description') }}</label>
                                    <textarea name="comment_en" class="form-control">{{ isset($review) ? $review->translate('en')->comment : '' }}</textarea>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="card card-flush">
                    <div class="card-header">
                        <div class="card-title">
                            <h3>{{ __('admin.global.rating') }}</h3>
                        </div>
                    </div>
                    <div class="card-body">
                        <div class="fv-row fv-plugins-icon-container">
                            <label class="required fs-6 fw-semibold mb-2">{{ __('admin.global.rating') }}</label>
                            <select class="form-select mb-2" id="rating" name="rating" data-kt-repeater="select2"
                                    data-placeholder="{{ __('admin.global.rating') }}" data-allow-clear="true">
                                <option value="1" @isset($review) {{ $review->rating == 1 ? 'selected' : '' }} @endisset>1</option>
                                <option value="2" @isset($review) {{ $review->rating == 2 ? 'selected' : '' }} @endisset>2</option>
                                <option value="3" @isset($review) {{ $review->rating == 3 ? 'selected' : '' }} @endisset>3</option>
                                <option value="4" @isset($review) {{ $review->rating == 4 ? 'selected' : '' }} @endisset>4</option>
                                <option value="5" @isset($review) {{ $review->rating == 5 ? 'selected' : '' }} @endisset>5</option>
                            </select>
                        </div>
                    </div>
                </div>
        ...
        <div class="page-buttuns mt-5">
            <div class="row justify-content-between">
                <div class="d-flex justify-content-end right">
                    ...
                    <a href="{{ route('admin.reviews.index') }}" id="kt_ecommerce_add_product_cancel"
